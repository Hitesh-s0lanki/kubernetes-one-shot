...
Install KIND and kubectl using the provided [script](/kind-cluster/install.sh):

sudo usermod -aG docker $USER && newgrp docker

## 2. Setting Up the KIND Cluster

Create a config.yaml file using the provided [script](/kind-cluster/config.yml):

Create the cluster using the configuration file: config.yml
